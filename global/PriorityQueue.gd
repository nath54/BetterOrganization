...
class PriorityQueue:
	...
	func remove(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(str(OS.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): remove(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return null
	...
	func merge_with(priority_queue, set_maintain_min = true):
		...
		var queue = self.PriorityQueue.new(list, set_maintain_min)
